    ...
    <div class="col-sm-12">
        <div class="card">
            <div class="card-header">
                <a href="<?= site_url('C_admin/add_alumni') ?>" class="btn btn-primary btn-flat">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr>
                                ...
                                <td><?= $d->nohp ?></td>
                                <td>
                                    <a class="btn btn-xs btn-warning" href="javascript:void(0)" onclick="status('<?= $d->id_user ?>','<?= $d->nama ?>')">
                                        <i class="fas fa-refresh"></i> <?= $status ?>
                                    </a>
                                </td>
                                <td class="text-center" width="100px">
                                    <a class="btn btn-xs btn-success" href="<?= site_url('C_admin/edit_alumni/' . $d->id_alumni) ?>">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a class="btn btn-xs btn-danger" href="javascript:void(0)" onclick="hapus('<?= $d->id_alumni ?>','<?= $d->id_user ?>','<?= $d->nama ?>')">
...
<script>
    function hapus(id_alumni, id_user, nama) {
        $('#hid').val(id_alumni);
        $('#hidu').val(id_user);
        $('#hnama').html(nama);
...
<div class="modal fade" id="hapus_data" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="<?php echo site_url('C_admin/delete_alumni') ?>">
                <div class="modal-body">
                    <input type="hidden" name="id_alumni" id="hid">
...
<div class="modal fade" id="status_user" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form method="POST" action="<?php echo site_url('C_admin/status_user_alumni') ?>">
                <div class="modal-body">
